...
import edu.nps.deep.be.avro.schemas.SplitData64M;
...
public class AvroConfig64M implements AvroApiConfig
{
  public static int PARTITIONSIZE = 1024*1024*64;
  private DataFileWriter<DiskImageSplit64M> diFileWriter;
  ...
  public void init(String inputPath, String outputPath) throws IOException
  {
    ...
    DatumWriter<DiskImageSplit64M> diDatumWriter = new SpecificDatumWriter<>(DiskImageSplit64M.class);
  ...
  {
    diFileWriter.create(new DiskImageSplit64M().getSchema(), filedata[OUTP].outputStream);
  }
  ...
  {
    DiskImageSplit64M dis = new DiskImageSplit64M(fileoffset,countRead,new SplitData64M(ba));
